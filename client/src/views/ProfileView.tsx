    ...
    return (
      <Stack spacing={1} sx={{ my: 2 }}>
        ...
        <Typography variant="body1" fontWeight="bold">
          {userQuery.data?.login ?? null}
        </Typography>
        <Typography variant="body2" color="text.secondary">
          Password:
        </Typography>
        <Typography variant="body1" fontWeight="bold">
          <Link onClick={() => setShowPasswordChange(true)} variant="body1">
            Change
          </Link>
        </Typography>
        <Typography variant="body2" color="text.secondary">
        ...
        <Typography variant="body1" fontWeight="bold">
          {userQuery.data?.Photos?.length ?? 0}
        </Typography>
        {userQuery.data?.Tags && userQuery.data?.Tags.length > 0 && (
          <>
            ...
            <Box
              ...
              component="ul"
            >
              {userQuery.data?.Tags?.map((tag) => (
                <ListItem key={tag.id}>
                  <Chip size="medium" label={tag.name} onDelete={() => handleTagClick(tag)} />
                </ListItem>
              ))}
            </Box>
          </>
        ...
        <Typography variant="body1" fontWeight="bold">
          {userQuery.data?.createdAt ? new Date(userQuery.data?.createdAt).toLocaleString() : null}
        </Typography>
